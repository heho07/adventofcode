DECLARE
	@InputString varchar(MAX) = '6-10 s: snkscgszxsssscss
6-7 b: bbbbbxkb
2-4 n: nnnjn
1-2 j: jjjj
5-9 z: jgzzzqhbj
...
SELECT
	LTRIM(RTRIM(X.value))
FROM
	string_split(@inputstring, CHAR(13)) X
...
(
	...
	FROM
		...
		(
			SELECT
				LTRIM(RTRIM(SUBSTRING(T.value, 1, CHARINDEX(' ', T.value)))) AS [rules],
				LTRIM(RTRIM(SUBSTRING(T.value, CHARINDEX(' ', T.value), CHARINDEX(' ', T.value, CHARINDEX(' ', T.value) + 1) - CHARINDEX(' ', T.value) - 1))) AS [allowedchar],
				LTRIM(RTRIM(SUBSTRING(T.value, CHARINDEX(' ', T.value, CHARINDEX(' ', T.value) + 1), LEN(T.value) - CHARINDEX(' ', T.value, CHARINDEX(' ', T.value))))) AS [password]
		) X
		CROSS APPLY
		(
			SELECT
				REPLACE(LTRIM(RTRIM(SUBSTRING(X.rules, 1, CHARINDEX('-', X.rules) - 1))), CHAR(10), '') AS [Minimum],
				REPLACE(LTRIM(RTRIM(SUBSTRING(X.rules, CHARINDEX('-', X.rules) + 1, LEN(X.rules) - CHARINDEX('-', X.rules) + 1))), CHAR(10), '') AS [Maximum]
		) Y
),
occurences AS
(
	SELECT
		cte.*,
		LEN(cte.password) - LEN(REPLACE(cte.password, cte.allowedchar, '')) AS [occurences],
		REPLACE(cte.password, cte.allowedchar, '') AS replaced
	FROM
		cte
)
SELECT
	--o.*,
	--IIF(
	--	o.occurences >= CONVERT(int, o.Minimum) AND
	--	o.occurences <= CONVERT(int, o.Maximum),
	--	1,
	--	0
	--) AS [Valid]
	COUNT(*)
...
(
	...
	FROM
		...
		(
			SELECT
				CONVERT(int, REPLACE(LTRIM(RTRIM(SUBSTRING(X.rules, 1, CHARINDEX('-', X.rules) - 1))), CHAR(10), '')) AS [PositionOne],
				CONVERT(int, REPLACE(LTRIM(RTRIM(SUBSTRING(X.rules, CHARINDEX('-', X.rules) + 1, LEN(X.rules) - CHARINDEX('-', X.rules) + 1))), CHAR(10), '')) AS [PositionTwo]
		) Y
),
occurences AS
(
	SELECT
		cte.*,
		IIF(SUBSTRING(cte.password, cte.PositionOne, 1) = cte.allowedchar, 1, 0) AS [MatchOnPositionOne],
		IIF(SUBSTRING(cte.password, cte.PositionTwo, 1) = cte.allowedchar, 1, 0) AS [MatchOnPositionTwo]
	FROM
		cte
),
results AS
...
SELECT
	o.*,
	IIF(
		o.MatchOnPositionOne = 1 AND
		o.MatchOnPositionTwo = 0
		OR
		o.MatchOnPositionOne = 0 AND
		o.MatchOnPositionTwo = 1,
		1,
		0
	) AS [Valid]
FROM
